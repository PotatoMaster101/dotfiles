...
[ -d "$HOME/.tools" ] && export PATH="$PATH:$HOME/.tools"
...
q() {
  rm -f "$HOME/.bash_history" "$HOME/.viminfo" "$HOME/.wget-hsts" "$HOME/.lesshst" "$HOME/.nano_history" "$HOME/.node_repl_history"
  rm -f "$HOME/.atftp_history" "$HOME/.mysql_history" "$HOME/.psql_history" "$HOME/.php_history" "$HOME/.python_history"
  rm -f "$HOME/.scapy_history" "$HOME/.recently-used" "$HOME/.xsession-errors" "$HOME/.xsession-errors.old"
  rm -f "$HOME/.kotlinc_history" "$HOME/.zsh_history" "$HOME/.zcompdump"*
  rm -rf "$HOME/.cache" "$HOME/.bash_sessions"
  history -p
  exit
}
